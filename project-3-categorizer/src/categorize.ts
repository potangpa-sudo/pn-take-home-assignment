...
export type Category = "food" | "transport" | "utilities" | "shopping" | "other";
...
const ALLOWED_CATEGORY_SET: ReadonlySet<string> = new Set(ALLOWED_CATEGORIES);

type CategorizeResultJson = {
  category?: unknown;
};
...
export function parseCategoryFromModelOutput(output: string): Category {
  ...
  const extracted =
    direct ?? safeJsonParse(extractFirstJsonObject(withoutFences) ?? "");

  const data = extracted as CategorizeResultJson | undefined;
  const raw = typeof data?.category === "string" ? data.category : "";
  const category = raw.trim().toLowerCase();
  return ALLOWED_CATEGORY_SET.has(category) ? (category as Category) : "other";
}

function ruleBasedCategory(description: string): Category | null {
  const d = description.toLowerCase();

  // Food / drinks / cafes
  if (
    /(กาแฟ|coffee|cafe|latte|americano|starbucks|cafe\s*amazon|ชา|tea|ข้าว|อาหาร|food|grabfood|lineman)/i.test(
      d,
    )
  ) {
    return "food";
...
function buildMessages(description: string) {
  ...
  const examples: Array<{ d: string; c: Category }> = [
  ...
  for (const ex of examples) {
    messages.push(
      { role: "user", content: `Transaction description: ${ex.d}` },
      { role: "assistant", content: JSON.stringify({ category: ex.c }) },
    );
  }
